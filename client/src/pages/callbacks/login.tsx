...
import { z } from "zod";
import { KeyRound, Loader2, Mail, PhoneCall } from "lucide-react";
...
export default function CallbacksLoginPage() {
  const { requestOtp, verifyOtp } = useCallbacksAuth();
  ...
  const codeForm = useForm<CodeForm>({ resolver: zodResolver(codeSchema), defaultValues: { code: "" } });
      ...
      <main className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-foreground mb-2">Callbacks Portal</h1>
            <p className="text-muted-foreground">Operator-only callback planning and chasing</p>
          </div>

          <Card className="shadow-lg">
            <CardHeader className="text-center pb-4">
              <CardTitle className="text-xl">{step === "email" ? "Sign In" : "Enter Code"}</CardTitle>
              <CardDescription>
                {step === "email" ? "Enter your operator email to receive a login code" : `Enter the 6-digit code sent to ${pendingEmail}`}
              </CardDescription>
            </CardHeader>
            <CardContent>
              {step === "email" ? (
                ...
                <Form {...codeForm}>
                  <form onSubmit={codeForm.handleSubmit(onCodeSubmit)} className="space-y-4" autoComplete="off">
                    <FormField
                    ...
                    <Button type="button" variant="ghost" className="w-full text-sm" onClick={() => setStep("email")} disabled={isBusy}>
